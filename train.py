...
if __name__ == '__main__':
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(dataloader):
            ...
            # 保存最佳网络结果
            if errG < loss_tep:
                torch.save(netG.state_dict(), "model/model_"+str(epoch+1)+"_"+str(iters)+".pt" )
                loss_tep = errG
            if(iters%250==0):
                ...
                plt.axis('off')  # 关闭坐标轴
                plt.savefig("picures_output/%d_%d.png" % (epoch+1, iters))
                plt.close(fig)
            iters+=1
        
        torch.save(netG.state_dict(), "model/model_"+str(epoch+1)+"_"+'0'+".pt" )
        print()
